...
public class DependencyResolver {
  ...
  private static final Pattern libNameParser =
  ...
  public Set<Plugin> getDeletions() { return deletions; }

  public Set<Plugin> getAdditions() { return additions; }

  public Map<String, String> getLibAdditions() { return libAdditions; }

  public Set<String> getLibDeletions() { return libDeletions; }
  ...
  private void resolveUpgrades() {
    ...
    for (Map.Entry<Plugin, Boolean> entry : allPlugins.entrySet()) {
      ...
      if (entry.getValue() && plugin.isInstalled() &&
  ...
  private void resolveUpdateLib(Plugin plugin, Library installedLib,
                                String candidateLibName) {
    final Map<String, String> candidateLibs =
        plugin.getLibs(plugin.getCandidateVersion());

    // get candidate lib
    Library candidateLib =
        getLibrary(candidateLibName, candidateLibs.get(candidateLibName));
    ...
    if (candidateLib.getVersion() != null &&
        Library.versionComparator.compare(installedLib, candidateLib) < 0) {
  ...
  private void resolveDeleteLibs() {
    ...
    for (Plugin plugin : allPlugins.keySet()) {
      if (additions.contains(plugin) ||
          (plugin.isInstalled() && !deletions.contains(plugin))) {
        String ver = additions.contains(plugin) ? plugin.getCandidateVersion()
        ...
        for (String lib : libs.keySet()) {
          ...
          if (libDeletions.contains(name)) {
            log.debug("Won't delete lib " + lib + " since it is used by " +
                      plugin);
  ...
  private void resolveUpgradesLibs() {
    for (Plugin plugin : deletions) {
      if (additions.contains(plugin)) { // if upgrade plugin
        final Map<String, String> installedLibs =
            plugin.getLibs(plugin.getInstalledVersion());
        final Map<String, String> candidateLibs =
            plugin.getLibs(plugin.getCandidateVersion());

        for (String candidateLibName : candidateLibs.keySet()) {
          String installedLibName =
              getMatchLibName(candidateLibName, installedLibs);
          if (installedLibName != null) {
            resolveUpdateLib(plugin,
                             getLibrary(installedLibName,
                                        installedLibs.get(installedLibName)),
                             candidateLibName);
          }
        }
      }
  ...
  private String getMatchLibName(String candidateLibName,
                                 Map<String, String> installedLibs) {
    final String candidateName = getLibName(candidateLibName);

    for (String installedLibName : installedLibs.keySet()) {
      if (installedLibName.startsWith(candidateName) &&
          getLibName(installedLibName).equals(candidateName)) {
  ...
  protected void verifyConditionFormat(String condition) {
    if (!condition.equals(">=")) {
      throw new IllegalArgumentException(
          "Expected conditions are ['>='], but was: " + condition);
    }
  }

  public void detectConflicts() {
    Set<Plugin> installedPlugins =
        PluginManager.getInstalledPlugins(allPlugins);

    for (Plugin plugin : installedPlugins) {
      Map<String, String> requiredLibs =
          plugin.getLibs(plugin.getInstalledVersion());
