...
public class PluginIcon {
  ...
  static {
    ...
    if (SVG_AVAILABLE) {
      try {
        com.github.weisj.darklaf.icons.IconLoader.class.getDeclaredMethod(
            "createFrameIcon", Icon.class, Window.class);
  ...
  public static Image getPluginFrameIcon(boolean hasUpdates, Window window) {
    if (FRAME_ICON_API_AVAILABLE) {
      return com.github.weisj.darklaf.icons.IconLoader.createFrameIcon(
          getPluginsIcon(hasUpdates), window);
    } else {
  ...
  private static Icon getPluginsSVGIcon(boolean hasUpdates, int size) {
    if (hasUpdates) {
      return com.github.weisj.darklaf.icons.IconLoader.get().getIcon(
          "org/jmeterplugins/logoUpdate.svg", size, size);
    } else {
      return com.github.weisj.darklaf.icons.IconLoader.get().getIcon(
          "org/jmeterplugins/logo.svg", size, size);
    }
  }

  private static ImageIcon getPluginImageIcon(boolean hasUpdates,
                                              boolean large) {
