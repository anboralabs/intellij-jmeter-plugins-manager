...
public class JARSourceHTTP extends JARSource {
  private static final Logger log =
      LoggerFactory.getLogger(JARSourceHTTP.class);
  ...
  private int timeout = Integer.parseInt(
      JMeterUtils.getPropDefault("jpgc.repo.timeout", "30000"));
  private final ServiceUnavailableRetryStrategy retryStrategy =
      new HttpRetryStrategy(RETRY_COUNT, 5000);
  ...
  private AbstractHttpClient getHTTPClient() {
    ...
    if (!proxyHost.isEmpty()) {
      int proxyPort =
          Integer.parseInt(System.getProperty("https.proxyPort", "-1"));
      ...
      String proxyUser =
          System.getProperty(JMeter.HTTP_PROXY_USER,
                             JMeterUtils.getProperty(JMeter.HTTP_PROXY_USER));
      if (proxyUser != null) {
        log.info("Using authenticated proxy with username: " + proxyUser);
        String proxyPass =
            System.getProperty(JMeter.HTTP_PROXY_PASS,
                               JMeterUtils.getProperty(JMeter.HTTP_PROXY_PASS));

        String localHost;
        try {
          localHost = InetAddress.getLocalHost().getCanonicalHostName();
        } catch (Throwable e) {
          log.error("Failed to get local host name, defaulting to 'localhost'",
                    e);
          localHost = "localhost";
        }
        ...
        NTCredentials credentials =
            new NTCredentials(proxyUser, proxyPass, localHost, proxyDomain);
  ...
  protected JSON getJSON(String uri) throws IOException {
    ...
    requestParams.setIntParameter(CoreConnectionPNames.CONNECTION_TIMEOUT,
                                  timeout);
    ...
    try {
      ...
      String response = isGZIPResponse(result) ? convertGZIPToString(bytes)
                                               : new String(bytes);
      int statusCode = result.getStatusLine().getStatusCode();
      if (statusCode >= 300) {
        log.warn("Response with code " + result + ": " + response);
        throw new IOException("Repository responded with wrong status code: " +
                              statusCode);
      } else {
  ...
  protected JSONArray getRepositories(String path) throws IOException {
    ...
    for (String address : addresses) {
      ...
      if (repo != null && repo.isActual()) {
        ...
        repositories.add(
            JSONSerializer.toJSON(repo.getRepoJSON(), new JsonConfig()));
      } else {
    ...
    for (JSON json : repositories) {
      ...
      for (Object elm : (JSONArray)json) {
        ...
        if (!pluginsIDs.contains(id)) {
          ...
          log.info("Plugin " + id +
                   " will be skipped, because it is duplicated.");
        }
      }
    }
    return result;
  ...
  public String getInstallID() {
    ...
    return getPlatformName() + '-' + DigestUtils.md5Hex(str.toString()) + '-' +
        getGuiMode();
  }
  ...
  protected String getPlatformName() {
    if (containsEnvironment("JENKINS_HOME")) {
      ...
      return "amazon";
    } else if (containsEnvironment("GOOGLE_APPLICATION_CREDENTIALS") ||
               containsEnvironment("CLOUDSDK_CONFIG")) {
      return "google_cloud";
  ...
  @Override
  public DownloadResult getJAR(final String id, String location,
    ...
    if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
      log.error("Error downloading url:" + url + " got response code:" +
    ...
    File tempFile =
        File.createTempFile(id.replaceAll("[^a-zA-Z0-9\\._]+", "_"), ".jar");
    ...
    try (InputStream inputStream = entity.getContent();
      ...
      copyLarge(inputStream, bos, new GenericCallback<Long>() {
        ...
        public void notify(Long progress) {
          callback.notify(
              String.format("Downloading %s: %d%%", id, 100 * progress / size));
        }
      });
    ...
    for (String uri : addresses) {
      ...
      try {
        ...
        HttpEntity body = new StringEntity(
            "stats=" +
            URLEncoder.encode(Arrays.toString(stats.toArray(new String[0])),
                              "UTF-8"));
        post.setEntity(body);
    ...
    if (repo != null) {
      ...
      if (hdr != null) {
        if (!repo.isActual(parseDateHeader(hdr))) {
          ...
          log.info("Cache file is not valid anymore, will drop it: " +
  ...
  private static long copyLarge(InputStream input, OutputStream output,
                                GenericCallback<Long> progressCallback)
      throws IOException {
