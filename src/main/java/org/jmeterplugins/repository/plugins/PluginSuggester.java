...
public class PluginSuggester {
  private static final Logger log =
      LoggerFactory.getLogger(PluginSuggester.class);
  ...
  public void checkAndSuggest(String msg) {
    ...
    if (pluginsToInstall.size() > 0) {
      ...
      SuggestDialog dialog =
          new SuggestDialog(null, pmgr, pluginsToInstall, testPlan);
  ...
  protected Set<Plugin> findPluginsFromClasses(Set<String> nonExistentClasses) {
    ...
    if (pluginsToInstall.isEmpty()) {
      log.warn(
          "Plugins Manager were unable to find plugins to satisfy Test Plan requirements. "
          +
  ...
  public TestPlanAnalyzer getAnalyzer() { return analyzer; }
