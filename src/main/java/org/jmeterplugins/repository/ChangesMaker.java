...
public class ChangesMaker {
  ...
  public ProcessBuilder getProcessBuilder(File moveFile, File installFile,
                                          File restartFile) throws IOException {
  ...
  private File getTempPmgrJAR() throws IOException {
    // String jarPath =
    // URLDecoder.decode(PluginManager.class.getProtectionDomain().getCodeSource().getLocation().getFile(),
    // "UTF-8");
  ...
  public File getRestartFile(LinkedList<String> additionalJMeterOptions)
      throws IOException {
  ...
  private String
  getJMeterStartCommand(LinkedList<String> additionalJMeterOptions) {
    StringBuilder cmd = new StringBuilder(
        JMeterUtils.getJMeterBinDir() + File.separator + "ApacheJMeter.jar\n");
  ...
  public File getInstallFile(Set<Plugin> plugins,
      ...
      for (Plugin plugin : plugins) {
        ...
        if (cls != null) {
          ...
          for (String lib : libs.keySet()) {
            Library.InstallationInfo libInfo =
                getLibForInstallLibs(lib, installLibs);
            if (libInfo != null) {
  ...
  protected Library.InstallationInfo
  getLibForInstallLibs(String lib, Set<Library.InstallationInfo> installLibs) {
  ...
  protected String generateLibPath(String libName)
  ...
  public File getMovementsFile(Set<Plugin> deletes, Set<Plugin> installs,
                               Set<Library.InstallationInfo> installLibs,
                               Set<String> libDeletions) throws IOException {
    ...
    try (PrintWriter out = new PrintWriter(file)) {
        ...
        for (String lib : libDeletions) {
          for (Plugin plugin : allPlugins.keySet()) {
            if (plugin.isInstalled() && plugin.getInstalledPath().equals(lib)) {
              log.warn("Cannot delete " + lib + " since it is part of plugin " +
                       plugin);
      ...
      for (Library.InstallationInfo libInfo : installLibs) {
        out.print(libInfo.getTmpPath() + "\t" +
                  generateLibPath(libInfo.getDestinationFileName()) + "\n");
      }
