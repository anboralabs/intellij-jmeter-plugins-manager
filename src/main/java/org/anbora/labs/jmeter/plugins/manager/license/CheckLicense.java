...
public class CheckLicense {
  /**
   * PRODUCT_CODE must be the same specified in plugin.xml inside the
   * <product-descriptor> tag
   */
  private static final String PRODUCT_CODE = "PJMETERPLUGINSM";
   ...
   * Null return value means the LicensingFacade object is not initialized yet
   * => one cannot say for sure does a valid license for the plugin exist or
   * not. The interpretation of the null value is up to plugin.
   */
  @Nullable
  public static Boolean isLicensed() {
    ...
    if (cstamp.startsWith(KEY_PREFIX)) {
      // the license is obtained via JetBrainsAccount or entered as an
      // activation code
  ...
  private static boolean isKeyValid(String key) {
    ...
    try {
      ...
      // the last parameter of 'createCertificate()' set to 'false' switches off
      // certificate expiration checks. This might be the case if the key is at
      // the same time a perpetual fallback license for older IDE versions. Here
      // it is only important that the key was signed with an authentic
      // JetBrains certificate.
      sig.initVerify(
          createCertificate(Base64.getMimeDecoder().decode(
                                certBase64.getBytes(StandardCharsets.UTF_8)),
                            Collections.emptySet(), false));
      final byte[] licenseBytes = Base64.getMimeDecoder().decode(
          licensePartBase64.getBytes(StandardCharsets.UTF_8));
      sig.update(licenseBytes);
      if (!sig.verify(Base64.getMimeDecoder().decode(
              signatureBase64.getBytes(StandardCharsets.UTF_8)))) {
        return false;
      }
      // Optional additional check: the licenseId corresponds to the licenseId
      // encoded in the signed license data The following is a 'least-effort'
      // code. It would be more accurate to parse json and then find there the
      // value of the attribute "licenseId"
      final String licenseData =
          new String(licenseBytes, StandardCharsets.UTF_8);
      return licenseData.contains("\"licenseId\":\"" + licenseId + "\"");
    } catch (Throwable e) {
      e.printStackTrace(); // For debug purposes only. Normally you will not
                           // want to print exception's trace to console
    }
    return false;
  }
  ...
  private static boolean isLicenseServerStampValid(String serverStamp) {
    try {
      ...
      final byte[] signatureBytes =
          base64.decode(parts[4].getBytes(StandardCharsets.UTF_8));
      final byte[] certBytes =
          base64.decode(parts[5].getBytes(StandardCharsets.UTF_8));
      final Collection<byte[]> intermediate = new ArrayList<>();
      for (int idx = 6; idx < parts.length; idx++) {
        intermediate.add(
            base64.decode(parts[idx].getBytes(StandardCharsets.UTF_8)));
      }

      final Signature sig = Signature.getInstance(signatureType);

      // the last parameter of 'createCertificate()' set to 'true' causes the
      // certificate to be checked for expiration. Expired certificates from a
      // license server cannot be trusted
      sig.initVerify(createCertificate(certBytes, intermediate, true));

      sig.update(
          (timeStamp + ":" + machineId).getBytes(StandardCharsets.UTF_8));
  ...
  createCertificate(byte[] certBytes, Collection<byte[]> intermediateCertsBytes,
                    boolean checkValidityAtCurrentDate) throws Exception {
    final CertificateFactory x509factory =
    ...
    for (byte[] bytes : intermediateCertsBytes) {
      allCerts.add(
          x509factory.generateCertificate(new ByteArrayInputStream(bytes)));
    }
    ...
    try {
      ...
      final PKIXBuilderParameters pkixParams =
          new PKIXBuilderParameters(trustAchors, selector);
      pkixParams.setRevocationEnabled(false);
      if (!checkValidityAtCurrentDate) {
        // deliberately check validity on the start date of cert validity
        // period, so that we do not depend on the actual moment when the check
        // is performed
        pkixParams.setDate(cert.getNotBefore());
      }
      pkixParams.addCertStore(CertStore.getInstance(
          "Collection", new CollectionCertStoreParameters(allCerts)));
